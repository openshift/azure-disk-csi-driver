...
`
	...
	for _, test := range tests {
		...
		cloud, err := GetCloudProviderFromClient(context.Background(), kubeClient, "azure-cloud-provider", "kube-system", test.userAgent, test.allowEmptyCloudConfig, false, false, -1)
		if ((err == nil) == (test.expectedErr == nil)) && !reflect.DeepEqual(err, test.expectedErr) && !strings.Contains(err.Error(), test.expectedErr.Error()) {
			t.Errorf("desc: %s,\n input: %q, GetCloudProvider err: %v, expectedErr: %v", test.desc, test.kubeconfig, err, test.expectedErr)
		}
		if cloud != nil {
